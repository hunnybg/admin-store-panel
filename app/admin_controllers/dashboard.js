            ...
            for(var i=0; i<=14; i++)
            {
                ...
                var group = {
                    $group: {
                        ...
                        rejected_by_vaiter: { $sum: {$cond:[{$eq: ["$order_status", NEW_ORDER_STATE.CANCELLATION_REJECTED] }, 1, 0]} },
                        cancelled_by_user: { $sum: {$cond: [{$eq: ["$order_status", NEW_ORDER_STATE.CANCELLATION_CONFIRMED] }, 1, 0]} }
                    }
                }
