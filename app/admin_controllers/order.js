...
exports.admin_orders = function (request_data, response_data) {
    utils.check_request_params(request_data.body, [], function (response) {
        if (response.success) {
            ...
            var order_condition = {$match: {$and: [ {$or: [ {'order_status_id': {$eq: ORDER_STATUS_ID.RUNNING}} , {'order_status_id': {$eq: ORDER_STATUS_ID.IDEAL}} , {'order_status_id': {$eq: ORDER_STATUS_ID.WAITING_FOR_CANCEL}} ]} ,{$or: [{'order_status_manage_id': {$ne: ORDER_STATUS_ID.COMPLETED}}, {'request_id': {$eq: null}}]} ]}};
