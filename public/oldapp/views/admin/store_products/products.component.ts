...
export interface AddItem{
    ...
    price: any,
    tax: any,
    item_available_from_time: String,
    item_available_till_time: String,
    calorie: any,
    is_visible_in_store:  Boolean,
...
export class AdminProductsComponent implements OnInit {
    ...
    selected_item_time_array: any[] = [];
    item_time_millisecond_array: any[] = [];
    item_open_time_error: number;
    item_close_time_error: number;
    ...
    ngOnInit() {
        ...
        this.add_item ={
            ...
            price: 0,
            tax: 0,
            item_available_from_time: '',
            item_available_till_time: '',
            calorie: '',
            is_visible_in_store:  true,
        ...
        this.helper.http.post(this.helper.POST_METHOD.GET_PRODUCT_LIST,{type: this.helper.ADMIN_DATA_ID.ADMIN, store_id:this.add_product.store_id, server_token:this.add_product.server_token}).map((res:Response) => res.json()).subscribe(res_data=>{
        ...
        this.helper.http.post(this.helper.POST_METHOD.GET_SPECIFICATION_GROUP, {type: this.helper.ADMIN_DATA_ID.ADMIN,
        ...
        this.helper.http.post(this.helper.POST_METHOD.GET_IMAGE_SETTING, {type: this.helper.ADMIN_DATA_ID.ADMIN,
    ...
    updateSpecification() {
        ...
        {
            this.myLoading=true;
            this.helper.http.post(this.helper.POST_METHOD.ADD_SPECIFICATION,{type: this.helper.ADMIN_DATA_ID.ADMIN, specification_group_id: this.selected_specification_group_id , specification_name :this.specification_name_array ,
                store_id:this.add_product.store_id, server_token:this.add_product.server_token}).map((res_data: Response) => res_data.json()) .subscribe(res_data => {
                    ...
                    if(res_data.success)
                    {
                        ...
                        var index = this.specification_group_list.findIndex((x)=>x._id==this.selected_specification_group_id);
                        if(index !== -1){
                            this.specification_group_list[index].list = res_data.specifications;
                        }
                        if(this.delete_specification_array.length > 0)
                        {
                            this.myLoading=true;
                            this.helper.http.post(this.helper.POST_METHOD.DELETE_SPECIFICATION,{type: this.helper.ADMIN_DATA_ID.ADMIN, specification_group_id: this.selected_specification_group_id , specification_id :this.delete_specification_array ,
                                store_id:this.add_product.store_id, server_token:this.add_product.server_token}).map((res_data: Response) => res_data.json()) .subscribe(res_data => {
    ...
    get_item_list(){
        this.myLoading = true;
        this.helper.http.post(this.helper.POST_METHOD.GET_STORE_PRODUCT_ITEM_LIST,{type: this.helper.ADMIN_DATA_ID.ADMIN, store_id:this.add_product.store_id, server_token:this.add_product.server_token,
                ...
                if(this.filtered_item_list.length>0){
                    ...
                    setTimeout(() => {
                        ...
                        jQuery('#open').on('change', (event, res_data) => {
                            ...
                            this.item_detail.item_available_from_time = open
                            ...
                            if (open >= close) {
                                this.item_open_time_error = 1
                            }
                            else {
                                this.item_open_time_error = 0
                            }

                            for (var i in this.item_time_millisecond_array) {

                                var open_time = this.item_time_millisecond_array[i].item_open_time;
                                var close_time = this.item_time_millisecond_array[i].item_close_time;
                        ...
                        jQuery('#close').on('change', (event, res_data) => {
                            this.item_close_time_error = 0;
                            this.item_open_time_error = 0;
                            var open = jQuery("#open").val()
                            var close = jQuery("#close").val();

                            this.item_detail.item_available_till_time = close
                            var open_date = new Date(Date.now())
                            open = open.split(':');
                            open_date.setHours(open[0], open[1], 0, 0)
                            open = open_date.getTime();
                            ...
                            for (var i in this.item_time_millisecond_array) {
                                let open_time = this.item_time_millisecond_array[i].item_open_time
                                let close_time = this.item_time_millisecond_array[i].item_close_time
    ...
    addiItemSpecification(data)
    {
        ...
        if(this.check_type_validation()) {
            ...
            let specification_id_array = this.specification_id_array.filter(function(elem, index, self) {
            ...
            if (size == 0) {
                ...
                for (var i = 0; i < size; i++) {
                    ...
                    for (var j = 0; j < product_size; j++) {
                        if (this.product_specification_list[j]._id === specification_id_array[i]) {
    ...
    {
        this.helper.http.post(this.helper.POST_METHOD.DELETE_ITEM_IMAGE,{type: this.helper.ADMIN_DATA_ID.ADMIN, store_id:this.add_product.store_id , server_token:this.add_product.server_token,
