...
export class ProductsComponent implements OnInit, AfterViewInit {
    ...
    import_data_modal: ModalComponent;
    ...
    ngAfterViewInit(): void {
        setTimeout(() => {
            // jQuery('#clock-picker1').clockpicker({
            //     'default': 'now',
            //     'align': 'right'
            // });
    ...
    import_data() {
        this.import_data_modal.open();
        // setTimeout(()=>{
        //     jQuery('#file_type').chosen();
        //     jQuery('#file_type').trigger("chosen:updated");
        //     jQuery(this.helper.elementRef.nativeElement).find('#file_type').on('change', (evnt,res_data) => {
        ...
        if (this.specification_name_array.length > 0) {
            ...
            this.helper.http.post(this.helper.POST_METHOD.ADD_SPECIFICATION, {
                ...
                if (res_data.success) {
                    ...
                    let index = this.specification_group_list.findIndex((x) => x._id == this.selected_specification_group_id);
                    if (index !== -1) {
    ...
    updateItem() {
        ...
        this.helper.http.post(this.helper.POST_METHOD.UPDATE_ITEM, this.item_detail).map((res: Response) => res.json()).subscribe(res_data => {
            ...
            if (res_data.success == true) {
                ...
                let index = this.filtered_item_list.findIndex((x) => x._id == this.item_detail._id);
                this.filtered_item_list[index] = JSON.parse(JSON.stringify(this.item_detail));
            }
            else {
                this.helper.data.storage = {
                    "message": this.helper.ERROR_CODE[res_data.error_code],
    ...
    change_time(item_from_time) {
        console.log(item_from_time)
    }
}
